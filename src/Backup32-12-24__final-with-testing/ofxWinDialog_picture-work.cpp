...
// Picture control
// Text is the path to a Windows bitmap (.bmp") image file
void ofxWinDialog::AddPicture(std::string text, int x, int y, int width, int height, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Static";
    control.Title = text;
    control.Text = text;
    control.Style = dwStyle;
    control.Index = 2; // To identify a picture from a hyperlink or static text
    control.X=x;
    control.Y=y;
    control.Width=width;
    control.Height=height;
...
HWND ofxWinDialog::Open(const char* windowTitle, int x, int y, int width, int height)
{
    ...
    for (size_t i=0; i<controls.size(); i++) {
        ...
        if (controls[i].Type == "Static") {
            ...
            // Picture control
            if (controls[i].Index == 2) {

                printf("Picture control x=%d y=%d w=%d, h= %d\n%s\n",
                    controls[i].X, controls[i].Y, controls[i].Width, controls[i].Height,
                    controls[i].Text.c_str());

                // Load a bitmap from a file
                // Path to a bmp image is specified by the text string
                HBITMAP hBitmap = (HBITMAP)LoadImageA(NULL, controls[i].Text.c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
                if (hBitmap) {

                    printf("Image loaded 0x%X\n", PtrToUint(hBitmap));

                    // Create the static control to display the bitmap
                    hwndc = CreateWindowExA(0, "STATIC", NULL,
                        WS_CHILD | WS_VISIBLE | SS_BITMAP, // Style for bitmap
                        controls[i].X, controls[i].Y, controls[i].Width, controls[i].Height,
                        hwnd, (HMENU)ID, m_hInstance, NULL);

                   if (hwndc) {
                       printf("Window created\n");
                       // Set the bitmap on the static control
                       SendMessage(hwndc, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBitmap);
                       controls[i].hwndControl = hwndc;
                       controls[i].hwndType = (HWND)hBitmap;
                       controls[i].ID = ID;
                       ID++;

                       // Re-size the bitmap
                       SendMessage(hwnd, WM_SIZE, 0, (LPARAM)hBitmap);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_CREATE:
            ...
            printf("WM_CREATE\n");
            for (size_t i=0; i<controls.size(); i++) {
                if (controls[i].Type == "Static" && controls[i].Index == 2) {
                    printf("Picture 1 LOWORD(wParam) = %d controls[i].ID = %d\n",
                        LOWORD(wParam), (int)controls[i].ID);
                    if (LOWORD(wParam) == controls[i].ID) {
                        printf("Picture 2\n");
                        // hwndType is the handle to the bitmap
                        // Resize the bitmap to fit the control
                        HDC hdcControl = GetDC(controls[i].hwndControl);
                        printf("  HDC = 0x%X\n", PtrToUint(hdcControl));

                        HDC hdcMem = CreateCompatibleDC(hdcControl);
                        printf("  hdcMem = 0x%X\n", PtrToUint(hdcMem));

                        printf("  controls[i].hwndType = 0x%X\n", PtrToUint(controls[i].hwndType));

                        HBITMAP hbmMem = (HBITMAP)SelectObject(hdcMem, controls[i].hwndType);
                        printf("  hbmMem = 0x%X\n", PtrToUint(hbmMem));

                        StretchBlt(hdcControl, 0, 0, controls[i].Width, controls[i].Height,
                            hdcMem, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
                            SRCCOPY);

                        // Clean up
                        SelectObject(hdcMem, hbmMem);
                        DeleteDC(hdcMem);
                        // ReleaseDC(controls[i].hwndControl, hdcControl);
                    }
            ...
            for (size_t i=0; i<controls.size(); i++) {
                ...
                if (controls[i].Type == "Static" && controls[i].Index == 2) {
                    ...
                    if (LOWORD(wParam) == controls[i].ID) {
                        ...
                        // hwndType is the handle to the bitmap
                        // Resize the bitmap to fit the control
                        HDC hdcControl = lpdis->hDC; // GetDC(controls[i].hwndControl);
                        printf("  HDC = 0x%X\n", PtrToUint(hdcControl));
                ...
                */
                if (controls[i].Type == "Static" && controls[i].Index == 1) {
                    if (LOWORD(wParam) == controls[i].ID) {
                        ...
                        printf("Hyperlink controls[i].ID = %d\n", (int)controls[i].ID);

                        // Title is the text displayed
                        SetTextColor(lpdis->hDC, RGB(40, 100, 190)); // Blue colour
            ...
            for (size_t i=0; i<controls.size(); i++) {
                if (controls[i].Type == "Static" && controls[i].Index == 2) {
                    ...
                    if (controls[i].hwndType) {
                        ...
                        StretchBlt(hDC, 0, 0, controls[i].Width, controls[i].Height,
                            hMemDC, 0, 0, 50, 50, SRCCOPY);
                        ...
                        bool bRet = StretchBlt(hdcControl, 0, 0, controls[i].Width, controls[i].Height,
                            hdcMem, 0, 0, 640, 480, SRCCOPY);
    ...
    if (nCode >= 0 && PM_REMOVE == wParam)
    {
        ...
        if ((lpMsg->message >= WM_KEYFIRST && lpMsg->message <= WM_KEYLAST))
        {
            if (IsDialogMessage(pThis->m_hDialog, lpMsg))
            {
                ...
                if (lpMsg->message == WM_KEYDOWN && (lpMsg->wParam == 37 || lpMsg->wParam == 39)) {
