...
// Hyperlink
void ofxWinDialog::AddUrl(std::string title, std::string text, int x, int y, int width, int height, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Static";
    control.Title = title;
    control.Text  = text;
    // Enable notifications and owner draw for underline
            ...
            for (size_t i=0; i<controls.size(); i++) {
                if (controls[i].Type == "Static" && controls[i].Index == 1) {
                    if (LOWORD(wParam) == controls[i].ID) {
                        SetTextColor(lpdis->hDC, RGB(50, 102, 204)); // RGB(5, 99, 193));
                        DrawTextA(lpdis->hDC, controls[i].Text.c_str(), -1, &lpdis->rcItem, DT_CENTER);
                        /*
                        // Measure the text size to position the underline correctly
                        SIZE textSize{};
                        GetTextExtentPoint32A(lpdis->hDC, controls[i].Text.c_str(), (int)controls[i].Text.length(), &textSize);
                        // Define the position for the text drawing
                        RECT textRect = lpdis->rcItem;
                        textRect.left += 2;  // Add some padding from the left edge
                        textRect.top += (textRect.bottom - textRect.top - textSize.cy)/2;  // Vertically center the text
                        textRect.right = textRect.left + textSize.cx;  // Set the right edge based on text width
                        textRect.bottom = textRect.top + textSize.cy*13/16;  // Set the bottom edge based on text height
                        // Horizontally center the text
                        textRect.left = (lpdis->rcItem.left + lpdis->rcItem.right - textSize.cx) / 2;
                        textRect.right = textRect.left + textSize.cx;
                        // Underline the text: position the underline just below the text
                        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(51, 102, 204)); // RGB(6, 69, 173)); // Blue pen for the underline
                        HPEN hOldPen = (HPEN)SelectObject(lpdis->hDC, hPen);
                        // Calculate the position of the underline
                        MoveToEx(lpdis->hDC, textRect.left, textRect.bottom, NULL);
                        LineTo(lpdis->hDC, textRect.right, textRect.bottom);
