...
HWND ofxWinDialog::Open(const char* windowTitle, int x, int y, int width, int height)
{
    ...
    // Set the window to be layered (transparent)
    SetWindowLong(hwnd, GWL_EXSTYLE, GetWindowLong(hwnd, GWL_EXSTYLE) | WS_EX_LAYERED);
    // Set transparency level
    // Black background alpha 0-255 (0 - transparent)
    SetLayeredWindowAttributes(hwnd, RGB(0, 0, 0), 100, LWA_COLORKEY | LWA_ALPHA);
    ...
    for (size_t i=0; i<controls.size(); i++) {
        ...
        if (controls[i].Type == "Checkbox") {
            ...
            if (hwndc) {
                LONG style = GetWindowLong(hwndc, GWL_EXSTYLE);
                style &= ~WS_EX_LAYERED;
                SetWindowLong(hwndc, GWL_EXSTYLE, style);
                SetWindowPos(hwndc, NULL, 0, 0, 0, 0, SWP_FRAMECHANGED);
                ...
                SetTextColor(hdc, RGB(255, 255, 255));  // Set text color to white (or any color you prefer)
                */

/*
    case WM_CTLCOLORSTATIC:
    case WM_CTLCOLOREDIT:
    case WM_CTLCOLOURLINK:
*/
            /*
            HDC hdc = GetDC(hwndc);
            // For dialog controls (static text, edit controls, etc.), return the default brush for normal painting
            SetBkMode((hdc)wParam, TRANSPARENT);
            SetTextColor((HDC)wParam, RGB(255, 255, 255));  // Set text color to white (or any color you prefer)
        ...
        if (controls[i].Type == "Slider") {
            ...
            if (hwndc) {
                // Disable visual style themes to avoid flat appearance
                // in case using Common-Controls Version 6
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_CREATE:
            ...
            SetWindowLong(hwnd, GWL_EXSTYLE, GetWindowLong(hwnd, GWL_EXSTYLE) | WS_EX_LAYERED);
            // Set transparency level
            // Black backgorud alpha 0-255 (0 - transparent)
            SetLayeredWindowAttributes(hwnd, RGB(0, 0, 0), 100, LWA_COLORKEY | LWA_ALPHA);
        ...
        case WM_ERASEBKGND:
            printf("Background\n");
            // Prevent default background erasing (keep transparency)
            return 1;
        ...
        // case WM_CTLCOLORDLG:
            {
                ...
                HDC hdc = (HDC)wParam;
                SetBkMode(hdc, OPAQUE); //  TRANSPARENT); // Set button background to transparent
                SetTextColor(hdc, RGB(255, 255, 255)); // Set button text color (optional)
                return (LRESULT)(HBRUSH)GetStockObject(NULL_BRUSH);
            }
        */
