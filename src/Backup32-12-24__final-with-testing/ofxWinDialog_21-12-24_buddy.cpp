...
struct ctl {
    ...
    // LJ DEBUG
    HWND hwndSpin = NULL; // Slider buddy window spin control
            ...
            if (hwndc) {
                ...
                if (controls[i].Index > 0) {
                    ...
                    HWND hwndval = CreateWindowExA(
                        ...
                        // WS_VISIBLE | WS_CHILD | SS_RIGHT, // right aligned
                        controls[i].X + controls[i].Width, controls[i].Y,
                        45, controls[i].Height, hwnd, NULL, m_hInstance, NULL);
                    if (hwndval) {

                        // Create the spin control (Up/Down buttons) to the right of the buddy window
                        HWND hSpin = CreateWindowExA(0, UPDOWN_CLASS, NULL,
                            WS_CHILD | WS_VISIBLE | UDS_ALIGNRIGHT | UDS_SETBUDDYINT,
                            controls[i].X+controls[i].Width+50, controls[i].Y - 3,
                            45, controls[i].Height, hwnd, NULL, m_hInstance, NULL);

                        // Set range for the spin control (same range as the trackbar)
                        // SendMessage(hSpin, UDM_SETRANGE, 0, MAKELONG(0, 100));
                        // SendMessage(hSpin, UDM_SETPOS, 0, MAKELONG(50, 0)); // Initial position
                        SendMessage(hSpin, TBM_SETRANGE, 0, MAKELONG((int)(controls[i].Min*100.0f), (int)(controls[i].Max*100.0f)));
                        SendMessage(hSpin, TBM_SETPOS, 0, MAKELONG((int)(controls[i].SliderVal*100.0f), 0));

                        SendMessage(hwndc, TBM_SETBUDDY, FALSE, (LPARAM)hwndval);

                        // SendMessage(hwndc, TBM_SETBUDDY, TRUE, (LPARAM)hwndval);
                        ...
                        // SendMessageA(hSpin, UDM_SETPOS, 0, MAKELONG(controls[i].SliderVal, 0));
                        // ???
                        SendMessageA(hSpin, UDM_SETPOS, 0, MAKELONG((int)(controls[i].SliderVal*100.0f), 0));

                        controls[i].hwndSliderVal = hwndval;
                        controls[i].hwndSpin = hSpin;
        ...
        case WM_NOTIFY:
            if (lParam)
            {
                ...
                if (controls.size() > 0) {
                    for (size_t i=0; i<controls.size(); i++) {
                        if (controls[i].Type == "Slider") {
                            // printf("WM_NOTIFY - Slider - (HWND)lParam = 0x%X, wParam = 0x%X\n",
                                // PtrToUint((HWND)lParam), (unsigned int)wParam);

                            if (wParam == 0) {
                                int spinPos = (int)LOWORD(((LPNMUPDOWN)lParam)->iPos);
                                int delta = ((LPNMUPDOWN)lParam)->iDelta; // Direction of the spin
                                printf("WM_NOTIFY - Spin spinPos = %d, delta = %d\n", spinPos, delta);

                                // Reverse the direction of the spin control
                                if (delta < 0)
                                    spinPos+=2; // Up button pressed, increment the value
                                else if (delta > 0)
                                    spinPos-=2; // Down button pressed, decrement the value
                                if (spinPos > 100) spinPos = 100;
                                ...
                                // LG DEBUG
                                SendMessage(controls[i].hwndControl, TBM_SETPOS, TRUE, spinPos*255);
                                controls[i].SliderVal = (float)spinPos*255;




                                // controls[i].SliderVal = 
                                // Slider value text display
                                if (controls[i].hwndSliderVal) {
                                    char tmp[8]{};
                                    if (controls[i].Max >= 100.0f)
                                        sprintf_s(tmp, 8, "%.0f", controls[i].SliderVal/100);
                                    else if (controls[i].Max >= 10.0f)
                                        sprintf_s(tmp, 8, "%.1f", controls[i].SliderVal/100);
                                    else
                                        sprintf_s(tmp, 8, "%.2f", controls[i].SliderVal/100);
                                    SetWindowTextA(controls[i].hwndSliderVal, (LPCSTR)tmp);
                                }
                                // Update the spin control's position
                                SendMessage(controls[i].hwndSpin, UDM_SETPOS, 0, MAKELONG(spinPos, 0));
                            }
                        }
                    }
                }
                ...
                for (size_t i=0; i<controls.size(); i++) {
                    if (controls[i].Type == "Slider") {
                        if ((HWND)lParam == controls[i].hwndControl) {
                            ...
                            // LJ DEBUG
                            // Update the spin control's position
                            SendMessage(controls[i].hwndSpin, UDM_SETPOS, 0, MAKELONG(pos, 0));
