...
// Get current control values and inform ofApp
void ofxWinDialog::GetControls()
{
    ...
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Edit") {
            ...
            printf("GetControls - slider [%s] controls[i].SliderVal = %.3f\n", controls[i].Title.c_str(), controls[i].SliderVal);
            pThis->DialogFunction(controls[i].Title, "", pos);
        }
    ...
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type    != "Static"
            ...
            if (controls[i].Type == "Combo") {
                ...
                printf("GetControls check - slider [%s] controls[i].SliderVal = %.3f, val = %d\n", controls[i].Title.c_str(), controls[i].SliderVal, (int)(controls[i].SliderVal*100.0f));
                // printf("GetControls check - slider [%s] controls[i].SliderVal = %.3f\n", controls[i].Title.c_str(), controls[i].SliderVal);
            }
            else if (controls[i].Type == "Edit") {
               // pThis->DialogFunction(controls[i].Title, controls[i].Text, true);
            }
            else {
                printf("controls[i].Type [%s] controls[i].Title [%s] - controls[i].Val = %d\n", controls[i].Type.c_str(), controls[i].Title.c_str(), controls[i].Val);
    ...
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type    != "Static"
            ...
            if (controls[i].Type == "Combo") {
                ...
                // LJ DEBUG
                printf("GetControls - slider [%s] controls[i].SliderVal = %.3f, val = %d\n", controls[i].Title.c_str(), controls[i].SliderVal, (int)(controls[i].SliderVal*100.0f));
                pThis->DialogFunction(controls[i].Title, "", (int)(controls[i].SliderVal*100.0f));
            }
            else if (controls[i].Type == "Edit") {
                // LJ DEBUG
                // printf("GetControls send - Edit [%s] controls[i].Text = %s\n", controls[i].Title.c_str(), controls[i].Text.c_str());
                pThis->DialogFunction(controls[i].Title, controls[i].Text, true);
            }
            else {
                // LJ DEBUG
                // printf("GetControls send - Title [%s] controls[i].Val = %d\n", controls[i].Title.c_str(), controls[i].Val);
                pThis->DialogFunction(controls[i].Title, "", controls[i].Val);
            }
        }
    }
...
void ofxWinDialog::Restore()
{
    /*
    // LJ DEBUG
    printf("Restore - current controls\n");
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type    != "Static"
            && controls[i].Type != "Group"
            && controls[i].Type != "Button"
            ...
            && controls[i].Type != "CANCEL") {
            if (controls[i].Type == "Combo")
                printf("Combo [%s] controls[i].Index = %d\n", controls[i].Title.c_str(), controls[i].Index);
            else if (controls[i].Type == "Slider")
                printf("Slider [%s] controls[i].SliderVal = %.3f, val = %d\n", controls[i].Title.c_str(), controls[i].SliderVal, (int)(controls[i].SliderVal*100.0f));
            else if (controls[i].Type == "Edit")
                printf("Edit [%s] controls[i].Text = %s\n", controls[i].Title.c_str(), controls[i].Text.c_str());
            else
                printf("Button [%s] controls[i].Val = %d\n", controls[i].Title.c_str(), controls[i].Val);
        }
    }
    */
    ...
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type    != "Static"
            ...
            if (controls[i].Type == "Combo")
                ...
                printf("Slider [%s] controls[i].SliderVal = %.3f, val = %d\n", controls[i].Title.c_str(), controls[i].SliderVal, (int)(controls[i].SliderVal*100.0f));
                printf("       [%s] controls[i].SliderVal = %.3f, val = %d\n", controls[i].Title.c_str(), controls[i].SliderVal, (int)(controls[i].SliderVal*100.0f));
            }
            else if (controls[i].Type == "Edit")
                printf("Edit [%s] controls[i].Text = %s\n", controls[i].Title.c_str(), controls[i].Text.c_str());
            else
...
void ofxWinDialog::Load(std::string filename, std::string section)
{
    ...
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type    != "Static"
            ...
            // LJ DEBUG
            // printf("Section [%s] : title [%s]\n", ControlSection.c_str(), (LPCSTR)controls[i].Title.c_str());
            ...
            if (controls[i].Type == "Combo") {
                ...
                if (GetPrivateProfileStringA((LPCSTR)ControlSection.c_str(), (LPCSTR)controls[i].Title.c_str(), NULL, (LPSTR)tmp, MAX_PATH, (LPCSTR)inipath.c_str())> 0) {
                    ...
                    // LJ DEBUG
                    // printf("Edit : Section [%s] : title [%s] = [%s]\n",
                        // ControlSection.c_str(), (LPCSTR)controls[i].Title.c_str(), controls[i].Text.c_str());
                }
            }
            else if (controls[i].Type == "Slider") {
                if (GetPrivateProfileStringA((LPCSTR)ControlSection.c_str(), (LPCSTR)controls[i].Title.c_str(), NULL, (LPSTR)tmp, MAX_PATH, (LPCSTR)inipath.c_str()) > 0) {
                    if (tmp[0]) controls[i].SliderVal = (float)atof(tmp);
                    // printf("Slider : Section [%s] (%s)\n", controls[i].Title.c_str(), tmp);
                }
            }
            else if (controls[i].Type == "Checkbox") {
                if (GetPrivateProfileStringA((LPCSTR)ControlSection.c_str(), (LPCSTR)controls[i].Title.c_str(), NULL, (LPSTR)tmp, MAX_PATH, (LPCSTR)inipath.c_str()) > 0) {
                    if (tmp[0]) controls[i].Val = atoi(tmp);
                    // printf("Checkbox : Section [%s] : title [%s] = %d\n", ControlSection.c_str(), controls[i].Title.c_str(), controls[i].Val);
                }
            }
        }
    }
