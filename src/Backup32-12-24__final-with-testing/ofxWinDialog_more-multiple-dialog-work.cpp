...
// WCHAR wClassName[MAX_PATH]{}; // Temporary name buffer for registration
...
ofxWinDialog::ofxWinDialog(ofApp* app, HINSTANCE hInstance, HWND hWnd, std::string classname) {
...
#ifdef UNICODE
    if (!classname.empty()) {
        mbstowcs_s(NULL, wClassName, classname.c_str(), MAX_PATH);
...
#else
    if (!classname.empty()) {
        strcpy_s(wClassName, MAX_PATH, classname.c_str());
        wndClass.lpszClassName = wClassname;
    }
    else {
        wndClass.lpszClassName = "ofxWinDialogClass";
    }
    printf("Register - wndClass.lpszClassName [%s]\n", wndClass.lpszClassName);
    ...
    // Clear controls for each instance
    controls.clear();
...
void ofxWinDialog::Create(ofApp* app, HINSTANCE hInstance, HWND hWnd, const std::string classname)
{
    m_hInstance = hInstance;
    m_hwnd = hWnd;
    ...
    // LJ DEBUG
    printf("ofxWinDialog::Create - pAppDialogFunction = 0x%X\n", pAppDialogFunction);


    // Initalize all variables in this class file
...
HWND ofxWinDialog::Open(const char* windowTitle, int x, int y, int width, int height)
{
    ...
    if (m_hDialog) {
        if (IsWindow(m_hDialog)) {
            if (FindWindowA(NULL, "windowTitle")) {
            ...
            /*
            // Compare the window class name with wClassName
            char className[MAX_PATH]{};
            if (GetClassNameA(m_hDialog, className, MAX_PATH)) {
                wchar_t wname[MAX_PATH]{};
                mbstowcs_s(NULL, wname, className, MAX_PATH);
                if (wcscmp(wname, wClassName) == 0) {
                    // Same class
                    return m_hDialog;
                }
            }
    ...
    // HWND hwnd = CreateWindow(NULL, title,
    ...
    // LJ DEBUG
    printf("Window create 0x%X (m_hDialog = 0x%X)\n", PtrToUint(hwnd), PtrToUint(m_hDialog));
    ...
    */
    ...
    if (m_hIcon) {
        ...
        m_hIcon = NULL; // Not needed any more
    }

    printf("hwndDialogs.size() = %d\n", (int)hwndDialogs.size());

    if (hwnd == m_hDialog) {
        printf("Same window\n");
        // Clear all window handles for repeat open of the same dialog
        printf("controls.size() = %d oldcontrols = %d\n", (int)controls.size(), (int)oldcontrols.size());
        for (size_t i=0; i<controls.size(); i++) {
        ...
        printf("Different window\n");
        printf("controls.size() = %d oldcontrols = %d\n", (int)controls.size(), (int)oldcontrols.size());
        hwndDialogs.push_back(hwnd);
    }
    ...
    // LJ DEBUG
    // for (size_t i=0; i<controls.size(); i++) {
    for (size_t i=oldcontrols.size(); i<controls.size(); i++) {
            ...
            if (hwndc) {
                ...
                if (controls[i].Items.size() > 0) {
                    for (size_t j = 0; j<controls[i].Items.size(); j++) {
                        // Item string is wide chars if Unicode
                        #ifdef UNICODE
                            wchar_t itemstr[MAX_PATH]{};
                            mbstowcs_s(NULL, itemstr, controls[i].Items[j].c_str(), MAX_PATH);
                        #else
                            char itemstr[MAX_PATH]{};
                            strcpy_s(itemstr, MAX_PATH, controls[i].Items[j].c_str());
                        #endif
                        SendMessage(hwndc, (UINT)CB_ADDSTRING, (WPARAM)0, (LPARAM)itemstr);
                    }
                }
...
void ofxWinDialog::AppDialogFunction(void(ofApp::* function)(std::string title, std::string text, int value))
{
    ...
    // LJ DEBUG
    printf("ofxWinDialog::AppDialogFunction - pAppDialogFunction = 0x%X\n", pAppDialogFunction);
}
