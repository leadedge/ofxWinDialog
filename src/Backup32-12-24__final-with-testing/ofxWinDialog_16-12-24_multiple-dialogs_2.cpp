...
ofxWinDialog::ofxWinDialog(ofApp* app, HINSTANCE hInstance, HWND hWnd, std::wstring wClass) {
    ...
    if (!wClass.empty()) {
        wClassName = wClass;
        wndClass.lpszClassName = wClass.c_str();
    }
    else
...
{
    if(wClassName.empty())
        return pThis->WProc(hwnd, msg, wParam, lParam);
    else
        return pThis->WProc2(hwnd, msg, wParam, lParam);
