...
ofxWinDialog2::ofxWinDialog2(ofApp* app, HINSTANCE hInstance, HWND hWnd) {
    ...
    wndClass.lpszClassName = L"ofxWinDialogClass2";
    wndClass.lpfnWndProc = WindowProc2; // Window procedure
...
// Set a custom font
void ofxWinDialog2::SetFont(std::string name, LONG height, LONG weight)
{
    fontname = name;
    fontheight = height;
    fontweight = weight;
...
void ofxWinDialog2::AddCheckBox(std::string title, std::string text, 
...
// Radio button group
void ofxWinDialog2::AddRadioGroup()
{
...
void ofxWinDialog2::AddRadioButton(std::string title, std::string text,
    int x, int y, int width, int height, bool bChecked, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Radio";
...
void ofxWinDialog2::SliderMode(bool bOnce)
{
    bOneClick = bOnce;
}
...
//
void ofxWinDialog2::AddSlider(std::string title,
...
// Edit control
void ofxWinDialog2::AddEdit(std::string title, int x, int y, int width, int height, std::string text, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Edit";
    control.Title = title;
...
void ofxWinDialog2::AddCombo(std::string title, int x, int y, int width, int height, std::vector<std::string> items, int index)
{
    ctl control{};
    control.Type = "Combo";
    control.Title = title;
...
void ofxWinDialog2::AddButton(std::string title, std::string text, int x, int y, int width, int height, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Button";
    control.Title = title;
...
void ofxWinDialog2::AddText(std::string title, int x, int y, int width, int height, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Static";
    control.Title = title;
...
// Hyperlink
void ofxWinDialog2::AddUrl(std::string title, std::string text, int x, int y, int width, int height, DWORD dwStyle)
{
    ctl control{};
    control.Type = "Static";
    control.Title = title;
    ...
    control.Index = 1; // To identify a url from static text
    control.X=x;
    control.Y=y;
    control.Width=width;
    control.Height=height;
...
// Static Group box
void ofxWinDialog2::AddGroup(std::string title, int x, int y, int width, int height)
{
    ctl control{};
    control.Type = "Group";
    control.Title = title;
...
// Get checkbox state
int ofxWinDialog2::GetCheckBox(std::string title)
{
    int state = 0;
...
int ofxWinDialog2::GetRadioButton(std::string title)
{
    int state = 0;
    for (size_t i=0; i<controls.size(); i++) {
...
// Get slider value
float ofxWinDialog2::GetSlider(std::string title)
{
    float value = 0.0f;
    for (size_t i=0; i<controls.size(); i++) {
...
std::string ofxWinDialog2::GetEdit(std::string title)
{
    std::string str;
    for (size_t i=0; i<controls.size(); i++) {
...
int ofxWinDialog2::GetComboItem(std::string title)
{
    int index = 0;
    for (size_t i=0; i<controls.size(); i++) {
...
std::string ofxWinDialog2::GetComboText(std::string title, int item)
{
    std::string str;
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Combo") {
...
void ofxWinDialog2::SetSection(std::string title, std::string section)
{
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Title == title) {
...
// Set checkbox state
void ofxWinDialog2::SetCheckBox(std::string title, int value)
{
    for (size_t i=0; i<controls.size(); i++) {
...
void ofxWinDialog2::SetRadioButton(std::string title, int value)
{
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Radio") {
            if (controls[i].Title == title) {
...
// Set slider value
void ofxWinDialog2::SetSlider(std::string title, float value)
{
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Slider") {
            if (controls[i].Title == title) {
...
void ofxWinDialog2::SetEdit(std::string title, std::string text)
{
    for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Edit") {
            if (controls[i].Title == title) {
...
void ofxWinDialog2::SetCombo(std::string title, int item)
{
     for (size_t i=0; i<controls.size(); i++) {
        if (controls[i].Type == "Combo") {
...
void ofxWinDialog2::GetControls()
{
    for (size_t i=0; i<controls.size(); i++) {
...
void ofxWinDialog2::Save(std::string filename, bool bOverWrite)
{
    char tmp[MAX_PATH]{};
    std::string inipath;
...
// ofApp calls GetControls to get the updated values
void ofxWinDialog2::Load(std::string filename, std::string section)
{
    char tmp[MAX_PATH]{};
    std::string inipath="";
...
// Dialog window icon
void ofxWinDialog2::SetIcon(HICON hIcon)
{
    m_hIcon = hIcon;
}

// Create dialog window
HWND ofxWinDialog2::Open(const char* windowTitle, int x, int y, int width, int height)
{
...
void ofxWinDialog2::AppDialogFunction(void(ofApp::* function)(std::string title, std::string text, int value))
{
    pAppDialogFunction = function;
}

// Pass back the dialog item title and state to ofApp
void ofxWinDialog2::DialogFunction(std::string title, std::string text, int value)
{
    if(pAppDialogFunction)
...
// Utility
//
std::string ofxWinDialog2::float2string(float number, int places)
{
    ...
    switch (msg) {
        ...
        case WM_DRAWITEM:
            ...
            for (size_t i=0; i<pThis->controls.size(); i++) {
                if (pThis->controls[i].Type == "Static" && pThis->controls[i].Index == 1) {
                    if (LOWORD(wParam) == pThis->controls[i].ID) {
                        // Blue colour
                        SetTextColor(lpdis->hDC, RGB(40, 100, 190)); // 50, 102, 204));
                        DrawTextA(lpdis->hDC, pThis->controls[i].Text.c_str(), -1, &lpdis->rcItem, DT_CENTER);
                        // Set a hand cursor
                        SetClassLongPtr(pThis->controls[i].hwndControl, GCLP_HCURSOR, (LONG_PTR)cursorHand);
                    }
                }
            }
            break;
         ...
         case WM_COMMAND:
             ...
             if (pThis->controls.size() > 0) {
                 ...
                 for (size_t i=0; i<pThis->controls.size(); i++) {
                     if (pThis->controls[i].Type == "Static" && pThis->controls[i].Index == 1) {
                         if (LOWORD(wParam) == pThis->controls[i].ID) {
                             ShellExecuteA(hwnd, "open", pThis->controls[i].Text.c_str(), NULL, NULL, SW_SHOWNORMAL);
                         }
                     }
                 }
