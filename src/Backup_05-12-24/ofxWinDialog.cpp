...
HWND ofxWinDialog::Open(const char* windowTitle, int x, int y, int width, int height)
{
    ...
    DWORD dwStyle = DS_3DLOOK | DS_CENTER | DS_MODALFRAME | DS_SHELLFONT | WS_CAPTION | WS_VISIBLE | WS_POPUP | WS_SYSMENU;
    if (!fontname.empty()) dwStyle |= DS_SETFONT;
    else dwStyle |= DS_SHELLFONT;
    HWND hwnd = CreateWindowW(wndClass.lpszClassName, (LPCWSTR)wstrTitle.c_str(), dwStyle,
